package cn.skuu.framework.websocket.core.sender.rabbitmq;
...
@RabbitListener(
        bindings = @QueueBinding(
                value = @Queue(
                        ...
                        name = "${skuu.websocket.sender-rabbitmq.queue}" + "-" + "#{T(java.util.UUID).randomUUID()}",
                        // Consumer 关闭时，该队列就可以被自动删除了
                        autoDelete = "true"
                ),
                exchange = @Exchange(
                        name = "${skuu.websocket.sender-rabbitmq.exchange}",
