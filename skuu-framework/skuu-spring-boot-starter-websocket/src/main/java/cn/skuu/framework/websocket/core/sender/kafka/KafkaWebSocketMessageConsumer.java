package cn.skuu.framework.websocket.core.sender.kafka;
...
public class KafkaWebSocketMessageConsumer {
    ...
    @RabbitHandler
    @KafkaListener(
            topics = "${skuu.websocket.sender-kafka.topic}",
            // 在 Group 上，使用 UUID 生成其后缀。这样，启动的 Consumer 的 Group 不同，以达到广播消费的目的
            groupId = "${skuu.websocket.sender-kafka.consumer-group}" + "-" + "#{T(java.util.UUID).randomUUID()}")
