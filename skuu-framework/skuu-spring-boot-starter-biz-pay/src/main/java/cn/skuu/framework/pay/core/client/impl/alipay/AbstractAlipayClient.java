...
import cn.skuu.framework.pay.core.client.impl.AbstractPayClient;
import cn.skuu.framework.pay.core.enums.PayNotifyRefundStatusEnum;
import cn.skuu.framework.pay.core.client.dto.*;
...
public abstract class AbstractAlipayClient extends AbstractPayClient<AlipayPayClientConfig> {
    ...
    public boolean verifyNotifyData(PayNotifyDataDTO notifyData) {
        ...
        try {
            ...
            log.error("[AlipayClient verifyNotifyData][(notify param is :{}) 验证失败]", JsonUtils.toJsonString(notifyData.getParams()), e);
        }
        return verifyResult;
    }
    ...
    protected PayCommonResult<PayRefundUnifiedRespDTO> doUnifiedRefund(PayRefundUnifiedReqDTO reqDTO)  {
        ...
        try {
            ...
            log.info("[doUnifiedRefund][response({}) 发起退款 渠道返回", JsonUtils.toJsonString(response));
            ...
            // TODO 记录异常日志
            log.error("[doUnifiedRefund][request({}) 发起退款失败,网络读超时，退款状态未知]", JsonUtils.toJsonString(reqDTO), e);
