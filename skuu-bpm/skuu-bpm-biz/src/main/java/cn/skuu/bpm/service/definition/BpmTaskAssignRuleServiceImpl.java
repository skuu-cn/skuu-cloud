...
import cn.skuu.bpm.controller.admin.definition.vo.rule.BpmTaskAssignRuleCreateReqVO;
import cn.skuu.bpm.controller.admin.definition.vo.rule.BpmTaskAssignRuleRespVO;
import cn.skuu.bpm.controller.admin.definition.vo.rule.BpmTaskAssignRuleUpdateReqVO;
import cn.skuu.bpm.convert.definition.BpmTaskAssignRuleConvert;
...
import cn.skuu.bpm.dal.mysql.definition.BpmTaskAssignRuleMapper;
import cn.skuu.bpm.enums.definition.BpmTaskAssignRuleTypeEnum;
import cn.skuu.bpm.enums.DictTypeConstants;
...
public class BpmTaskAssignRuleServiceImpl implements BpmTaskAssignRuleService {
    ...
    public void updateTaskAssignRule(@Valid BpmTaskAssignRuleUpdateReqVO reqVO) {
        ...
        if (!Objects.equals(BpmTaskAssignRuleDO.PROCESS_DEFINITION_ID_NULL, existRule.getProcessDefinitionId())) {
            throw exception(ErrorCodeConstants.TASK_UPDATE_FAIL_NOT_MODEL);
        }
    ...
    Set<Long> calculateTaskCandidateUsers(DelegateExecution execution, BpmTaskAssignRuleDO rule) {
        ...
        if (CollUtil.isEmpty(assigneeUserIds)) {
            ...
            throw exception(ErrorCodeConstants.TASK_CREATE_FAIL_NO_CANDIDATE_USER);
        }
    ...
    private Set<Long> calculateTaskCandidateUsersByScript(DelegateExecution execution, BpmTaskAssignRuleDO rule) {
        ...
        rule.getOptions().forEach(id -> {
            ...
            if (script == null) {
                throw exception(ErrorCodeConstants.TASK_ASSIGN_SCRIPT_NOT_EXISTS, id);
            }
            scripts.add(script);
        });
        // 逐个计算任务
